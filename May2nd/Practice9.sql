-- SUBQUERIES.
-- Subqueries, as implied by the name, are queries made inside a query.


-- Let's start by using a subquery in a WHERE clause.
...
-- Now we can complicated the subquery by using it in a WHERE with a DELETE
...
-- Can we use subqueries as a derived table in a FROM clause? Let's see.
...
-- Now let's try adding a subquery to a column list.
...
-- So far, subqueries have proven to be amazing. They can be used almost everywhere so far.
-- Can subqueries be sed for math and calcultions.
...
-- And now for creating and filling a retirees table.
...
-- And finally, let's try using a subquery with a LATERAL join.
...
-- Starting this section, let's a simple CTE to count large counties.
...
-- And that should be all for today's Practice #9
